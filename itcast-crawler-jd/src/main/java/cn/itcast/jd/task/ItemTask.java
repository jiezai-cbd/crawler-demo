...
public class ItemTask {
    ...
    //解析页面，获取商品数据并存储
    private void parse_backup(String html) throws Exception {
        //解析html获取Document
    ...
    private void parse(String html) throws Exception {
        ...
        for (Element spuEle : spuEles) {
            ...
            //for (Element skuEle : skuEles) {
                //获取sku
                //long sku = Long.parseLong(spuEle.select("[data-sku]").attr("data-sku"));
                long sku = spu;
                //根据sku查询商品数据
                Item item = new Item();
                item.setSku(sku);
                ...
                //获取商品的图片
                String picUrl ="https:"+ spuEle.select("img").first().attr("src");
                //picUrl = picUrl.replace("/n9/","/n1/");
                String picName = this.httpUtils.doGetImage(picUrl);
                item.setPic(picName);
